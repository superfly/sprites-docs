...
receive do
  ...
  {:stderr, ^cmd, data} -> IO.write(:stderr, data)
  {:exit, ^cmd, code} -> IO.puts("Exited: #{code}")
...
| `{:port, command, port}` | Port assignment (TTY mode) |
...
```elixir
alias Sprites.Error.{APIError, CommandError, ConnectionError, TimeoutError}
...
| Error | Description |
|-------|-------------|
| `Sprites.Error.APIError` | API returned an error response |
| `Sprites.Error.CommandError` | Command exited with non-zero code |
| `Sprites.Error.ConnectionError` | WebSocket connection failed |
| `Sprites.Error.TimeoutError` | Operation timed out |
