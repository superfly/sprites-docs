...
- **Anthropic, Meta, OpenAI, and other API-key providers** — paste the key. It's validated against the provider before it's saved, encrypted at rest, and never shown again.
...
- **Rate-limit headers do come back.** GitHub, OpenRouter, Anthropic, Meta, OpenAI, and Slack connectors forward their `*-ratelimit-*` and `retry-after` response headers. Slack connectors also return `x-slack-req-id`, `x-oauth-scopes`, and `x-accepted-oauth-scopes`, which is handy for diagnosing a missing scope.
...
The same settings work as one-off flags: `codex exec -c model_provider=sprites -c model_providers.sprites.base_url=... -c model_providers.sprites.env_key=OPENAI_API_KEY -c model_providers.sprites.wire_api=responses "..."`.
...
Works for `openrouter`, `anthropic`, `ollama`, `meta`, and `openai`. The key is validated against the provider before it's stored. Rotate it later with `PUT /v1/oauth/connections/<id>/api_key`.
