...
| Ollama | API key (BYOK) | Ollama Cloud. Both native `/api/*` routes and OpenAI-compatible `/v1/*` routes work. |
...
Some providers, such as Slack and Ollama, are enabled per organization. If a provider you expect isn't in the **Add connector** picker, it isn't switched on for your organization yet. Existing connections keep working regardless.
</Callout>
...
Click **Add connector** to pick a provider. What happens next depends on the provider:
...
- **Anthropic, Ollama, and other API-key providers** — paste the key. It's validated against the provider before it's saved, encrypted at rest, and never shown again.
...
- **Responses are buffered.** The gateway reads the whole provider response before returning it, so streaming doesn't stream. For Ollama's native chat API, which streams by default, set `"stream": false`. The same applies to `stream: true` on OpenAI-compatible and Anthropic requests.
...
Works for `openrouter`, `anthropic`, and `ollama`. The key is validated against the provider before it's stored. Rotate it later with `PUT /v1/oauth/connections/<id>/api_key`.
