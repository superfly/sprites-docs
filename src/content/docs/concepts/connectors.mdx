...
| --- | --- | --- |
| GitHub | OAuth | You approve access on GitHub. Default scopes are `repo` and `read:org`; add more later from the connector's Scopes editor. The token reaches every repository you can see. |
| GitHub App | GitHub App | You choose an account and the repositories on GitHub's install screen, which also authorizes. Sprites reach only those repositories, with the App's fine-grained permissions. There are no scopes. Tokens rotate every eight hours. |
...
- **GitHub App** — click **Continue with GitHub**. GitHub's install screen asks for the account and the repositories the App may reach, authorizes, and returns you to your connectors. The connector page lists each installation with its repositories and permissions, read from GitHub each time the page opens, so a changed selection on GitHub shows up at once.
