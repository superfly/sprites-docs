...
Works for `openrouter`, `anthropic`, and `meta`. The key is validated against the provider before it's stored. Rotate it later with `PUT /v1/oauth/connections/<id>/api_key`.
