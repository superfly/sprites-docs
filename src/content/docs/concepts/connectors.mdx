...
| Meta | API key (BYOK) | Muse Spark models through the Meta Model API. Forwards the OpenAI-compatible `/v1/chat/completions` and `/v1/responses` routes, the Anthropic-compatible `/v1/messages` route, and `/v1/models`; every other Meta path is rejected. See [Meta](#meta). |
...
- **Anthropic, Meta, and other API-key providers** — paste the key. It's validated against the provider before it's saved, encrypted at rest, and never shown again.
...
A Meta connector accepts the same request shapes as the provider it emulates. The OpenAI SDK pointed at `.../v1/gateway/meta/<connection_id>/v1` and the Anthropic SDK pointed at the same base URL both work with any placeholder API key, because the gateway replaces whatever key the SDK sends with the stored one. See [Meta](#meta) for details.
...
- **Rate-limit headers do come back.** GitHub, OpenRouter, Anthropic, Meta, and Slack connectors forward their `*-ratelimit-*` and `retry-after` response headers. Slack connectors also return `x-slack-req-id`, `x-oauth-scopes`, and `x-accepted-oauth-scopes`, which is handy for diagnosing a missing scope.
...
The Meta connector gives Sprites access to Meta's Muse Spark models through the [Meta Model API](https://dev.meta.ai). It's bring-your-own-key: create an API key in the Meta developer console (keys start with `LLM_`), paste it into **Add connector → Meta**, and the gateway validates it against Meta's model list before saving it.

Meta serves three inference surfaces with the same key, and the connector forwards all of them:

| Path | Compatible with |
| --- | --- |
| `/v1/chat/completions` | OpenAI Chat Completions API and SDKs |
| `/v1/responses` | OpenAI Responses API and SDKs |
| `/v1/messages` | Anthropic Messages API and SDKs. `anthropic-version` and `anthropic-beta` headers are forwarded when you send them. |
| `/v1/models` | Model discovery |

Anything else under Meta's API returns `403 endpoint is not supported by the Meta connector`, before the request leaves the gateway and regardless of the connector's access policy. That keeps an organization-level credential from becoming general account access.

Point an SDK at the connector by setting its base URL to the gateway and giving it any placeholder key:

```python
from openai import OpenAI

client = OpenAI(
    base_url="https://api.sprites.dev/v1/gateway/meta/<connection_id>/v1",
    api_key="sprites-gateway",  # replaced by the stored credential
)
...
The same works with the Anthropic SDK against `/v1/messages`, and with `curl`:
...
Streaming (`"stream": true`) and tool calling pass through unchanged on both the OpenAI-compatible and Anthropic-compatible routes, and Meta's `x-ratelimit-*`, `x-request-id`, and `retry-after` response headers come back to the Sprite.

A few Meta-specific notes:

- **Check which models your key can use.** Model availability is set per Meta developer team, so `GET /v1/models` through the connector is the authoritative list. `muse-spark-1.3` is Meta's documented default; if it's missing from your list, the key's team doesn't have access to it yet.
- **Muse Spark is a reasoning model.** Reasoning tokens count against `max_tokens`, so very small budgets can return an empty message with usage recorded. Give it room.
- **Point coding agents at the gateway, not at Meta.** Muse Code and other agents that read an OpenAI- or Anthropic-style base URL from configuration can use the connector URL with a placeholder key, so the Meta key never has to be written into an agent config file inside the Sprite.
...
Works for `openrouter`, `anthropic`, `ollama`, and `meta`. The key is validated against the provider before it's stored. Rotate it later with `PUT /v1/oauth/connections/<id>/api_key`.
