...
| Sprite is created, but the session dies within seconds | `pip install anthropic` failed inside the Sprite, for example a network policy blocking PyPI, so `spawn()` raises `runner setup failed (exit N)`. Or the environment key is invalid, so the runner exits immediately. | Both surface in the service log; see the row below for where to look. |
