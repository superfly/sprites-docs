...
- A **worker runtime** on the worker host: Python 3.10+ with the `anthropic` and `sprites-py` packages, or Node 22+ with `@anthropic-ai/sdk`, `@fly/sprites` and `tsx`.
...
def find_live(session_id) -> str | None:
    ...
    try:
        ...
        return None
    return name if sprite.status in ("running", "warm") else None
...
def spawn(session_id, *, environment_id, work_id, environment_key) -> str:
    ...
    try:
        ...
        # Another delivery for this session got here first; reuse its Sprite.
        sprite = client.sprite(name)

    fs = sprite.filesystem("/")
    (fs / RUNNER_PATH).write_text(RUNNER_SRC)

    # Install the SDK and create the workspace. This runs on every new Sprite;
    # the checkpoint note at the end of the guide covers cutting the cost. A
    # nonzero exit means the install failed (for example PyPI is blocked).
    result = sprite.run(
        "bash", "-c", f"mkdir -p {WORKDIR} && python3 -m pip install -q anthropic",
        capture_output=True,
    )
    if result.returncode != 0:
        raise RuntimeError(f"sprite {name}: runner setup failed (exit {result.returncode})")

    # The env contract Anthropic's worker reads. The environment key is the
    # runner's only credential. It is written to a file the service sources
    # rather than passed as service arguments, so it never appears in process
    # listings; the service deletes the file right after sourcing it.
    env = {
    ...
    # create_service streams startup events; draining it waits for the service
    # to actually come up rather than returning as soon as it is registered.
    for _event in sprite.create_service("agent-runner", cmd="bash", args=["-lc", cmd], needs=[]):
        pass
    return name
```
</TabItem>
...
const client = new SpritesClient(requireEnv('SPRITE_TOKEN'));

// Anthropic's provider-agnostic runner. One call is the whole worker: it builds
// the per-session tool context, downloads skills, runs the tool loop, and posts
// results back. This is the same Python runner the Python tab uploads; the
// worker's own language is independent of it.
...
export async function findLive(sessionId: string): Promise<string | null> {
  ...
  try {
    ...
    return sprite.status === 'running' || sprite.status === 'warm' ? name : null;
  } catch (e) {
    // A missing Sprite is the normal case for new work; anything else is real.
...
export async function spawn(sessionId: string, opts: SpawnParams): Promise<string> {
  ...
  let sprite;
  try {
    sprite = await client.createSprite(name, { waitForCapacity: true });
  } catch {
    // Another delivery for this session got here first; reuse its Sprite.
    sprite = client.sprite(name);
  }
  ...
  await fs.writeFile(RUNNER_PATH, RUNNER_SRC);

  // Install the SDK and create the workspace. This runs on every new Sprite; the
  // checkpoint note at the end of the guide covers cutting the cost. execFile is
  // what reports the command's real exit code — exec() reports 1 for any
  // failure, which would turn "PyPI is blocked" into an unhelpful message.
  try {
    await sprite.execFile('bash', ['-c', `mkdir -p ${WORKDIR} && python3 -m pip install -q anthropic`]);
  } catch (e) {
    if (e instanceof ExecError) {
      throw new Error(`sprite ${name}: runner setup failed (exit ${e.exitCode})`);
    }
    throw e;
  }
  ...
  await fs.writeFile('/root/runner.env', envFile);

  // createService streams startup events; draining it waits for the service to
  // actually come up rather than returning as soon as it is registered.
  const stream = await sprite.createService('agent-runner', {
    cmd: 'bash',
    args: ['-lc', SERVICE_CMD],
    needs: [],
  });
...
<TabItem label="Python">
```bash
pip install anthropic sprites-py
python sprites_poller.py
```
</TabItem>
<TabItem label="TypeScript">
```bash
npm install @anthropic-ai/sdk @fly/sprites tsx
...
- [`sprites-py`](https://github.com/superfly/sprites-py) and [`@fly/sprites`](https://github.com/superfly/sprites-js): the SDKs the worker uses.
- [Sprites API](/api/dev-latest/sprites): the underlying endpoints, for workers written in another language.
