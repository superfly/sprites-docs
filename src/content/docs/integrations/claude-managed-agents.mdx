...
def spawn(session_id, *, environment_id, work_id, environment_key) -> str:
    ...
    c.put(f"/v1/sprites/{name}/services/agent-runner",
          json={"cmd": "bash", "args": ["-lc", cmd], "needs": []})
    return name
```
...
async def main():
    ...
    async for work in client.beta.environments.work.poller(
        ...
        # "session" is the discriminator on the work item's data (the only
        # work-data type today); each carries the session id to hand to a Sprite.
...
The file the agent writes lands in `/workspace` inside the Sprite, which persists for the Sprite's lifetime. `/workspace` is this guide's own convention; Anthropic's harness normally directs final deliverables to `/mnt/session/outputs`, and it's the runner's `unrestricted_paths=True` that lets the tools read and write `/workspace` here.
