...
export function SnippetSelector({ languages, defaultLanguage }: SnippetSelectorProps) {
  ...
  const updateActiveBlock = (newValue: string) => {
    ...
    if (container) {
      ...
      const targetBlock = container.querySelector(`[data-language="${newValue}"]`);
      if (targetBlock) {
  ...
  useEffect(() => {
    ...
    document.addEventListener('language-preference-changed', handleGlobalChange);
    return () => {
      document.removeEventListener('language-preference-changed', handleGlobalChange);
    };
  }, [languages, value]);
