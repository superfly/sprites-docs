...
export function SelfHostedDiagram({ className }: { className?: string }) {
  ...
  const Badge = ({
    ...
    <g>
      <circle cx={x} cy={y} r="9" fill={`url(#shd-${tone})`} />
      <text
        x={x}
        y={y + 3.8}
        textAnchor="middle"
    ...
    >
      ...
      >
        <g transform="translate(0 66)">
          <circle r="32" fill="none" stroke="var(--shd-hair)" strokeWidth="2" />
          <motion.g
        ...
        {[0, 1, 2].map((i) => {
          ...
          return i === 0 && !reduce ? (
            <motion.g
              ...
              transition={{ duration: 3, times: [0, 0.22, 0.85, 1], ...REPEAT }}
            >
              {bar}
            </motion.g>
          ) : (
      ...
      >
        ...
        <text
          ...
          fill="var(--shd-sub)"
        >
          everything the tools touch stays in the Sprite
        </text>
      </motion.g>
