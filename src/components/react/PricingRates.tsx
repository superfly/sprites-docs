  ...
  return (
    <div className="my-6 space-y-3">
      <div className="flex items-center justify-end gap-2 text-sm">
        <span className={perSecond ? 'text-[var(--sl-color-gray-2)]' : 'text-[var(--sl-color-white)] font-medium'}>
          Hourly
        </span>
        <Switch checked={perSecond} onCheckedChange={setPerSecond} />
        <span className={perSecond ? 'text-[var(--sl-color-white)] font-medium' : 'text-[var(--sl-color-gray-2)]'}>
          Per Second
        </span>
      </div>
...
function RateCard({ title, rate, unit, description }: {
  title: string
  rate: string
  unit: string
  description?: string
  ...
  return (
    <div className="border border-[var(--sl-color-hairline)] bg-[var(--sl-color-bg-sidebar)] p-4">
      ...
      <div className="flex items-baseline gap-1.5 mt-1">
        <span className="text-2xl font-bold text-[var(--sl-color-accent)] font-mono tabular-nums">{rate}</span>
        <span className="text-sm text-[var(--sl-color-gray-2)]">{unit}</span>
      </div>
      {description && (
        <p className="text-xs text-[var(--sl-color-gray-2)] mt-2">{description}</p>
      )}
    </div>
  )
}
